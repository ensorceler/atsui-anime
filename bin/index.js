...
// banner statement
lolcatjs.fromString(figlet.textSync("Atsui-Anime", {
    font: "Slant",
...
program
    ...
    .option("-l,--limit <limit_no>", "limit number of animes in search results, default is 5")
    .description("search for an anime by specified name")
    .action((anime_name, options) => {
    if ("limit" in options) {
        let limit_no = options.limit;
        limit_no = limit_no;
...
program
    .command("getseason")
    .option("-l,--limit <limit_no>", "limit how many animes can appear in your search result ,default is 20")
    .option("-y,--year <year>", "specify the year")
    .option("-s,--season <season_type>", "specify the season")
    .description(" get currently airing anime, [if you specify the year ,season has to specified as well and vice-versa]")
    .action((options) => {
    // default option
